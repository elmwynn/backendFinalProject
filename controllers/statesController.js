...
const getStateData = async (req,res) => {
    const dataHolder = data.states.find(st => st.code === (req.params.state));
...
const createNewFunFact = async (req,res) =>{
    if(req.params.parameter === 'funfact'){
        try{
            const alreadyExists = await State.find({ stateCode: req.params.state}).exec();
            if(alreadyExists.length === 0){
                ...
                const filter = {stateCode: req.params.state};
                const update = {$push: {funfacts: req.body.funfacts}};
                await State.findOneAndUpdate(
                    filter, 
                    update
                );
                const result = await State.find({ stateCode: req.params.state}).exec();
                res.status(201).json(result);
            }
        } catch (err){
            console.error(err);
...
const deleteFunFact = async(req,res) =>{
    if(req.params.parameter === 'funfact'){
        try{
            const dataHolder = data.states.find(st => st.code === (req.params.state));
            const stateCheck = await State.find({ stateCode: req.params.state}).exec();
            if(!req?.body?.index || req.body.index === 0){
                ...
                const filter =  req.params.state;
                const oneToDelete = stateCheck[0].funfacts[req.body.index-1];
                await State.findOneAndUpdate({
                    filter,
                    $pull: {funfacts: oneToDelete}
                });
                const afterUpdate = await State.find({ stateCode: req.params.state}).exec();
